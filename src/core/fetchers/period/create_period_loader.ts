...
/** Response event from `createPeriodLoader`. */
export interface IPeriodLoaderResponse { type : "response";
                                         value : IPeriodLoaderResponseValue; }

// Events a loader emits
export type IPeriodLoaderEvent = IPeriodLoaderResponse |
...
export default function createPeriodLoader(
  ...
  return function loadPeriod(
    loaderArgs : IPeriodLoaderArguments
      ...
      return loadData(loaderArgs).pipe(
        mergeMap((arg) => {
          if (arg.type === "warning") {
